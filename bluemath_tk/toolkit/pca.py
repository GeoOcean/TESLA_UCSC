    ...
    def generate_dataset(self):
        ...
        for ivar, var in enumerate(self.predictor):
            ...
            if area[1]<area[0]: #Area goes over 0
                ds = ds.sel(latitude = slice(area[2]-area_buffer, area[3]+area_buffer))
                ds = ds.isel(longitude = np.where((ds.longitude.values>area[0]-area_buffer) | (ds.longitude.values<area[1]+area_buffer))[0])
            else:
                ds = ds.sel(longitude = slice(area[0]-area_buffer, area[1]+area_buffer,),
                            latitude = slice(area[2]-area_buffer, area[3]+area_buffer))
        ...
        if area[1]<area[0]: #Area goes over 0
            DS = DS.sel(latitude = slice(area[2], area[3]))
            ...
            DS = DS.sel(longitude = slice(area[0], area[1],),
    ...
    def plot_map_mean(self):
        ...
        for iv, var in enumerate(vars):
            ...
            if not estela: 
                ...
                lon_max = np.nanmin([359.9, ds.longitude.values.max()+5])
                lon_min = np.nanmax([0.1, ds.longitude.values.min()-5])
                print(lon_min)
                print(lon_max)
                ax.set_extent([lon_min, lon_max, 
                              ds.latitude.values.min()-5, ds.latitude.values.max()+5, ], crs=ccrs.PlateCarree())
                print(ds.latitude.values.min()-5)
                print(ds.latitude.values.max()+5)
                
            else:                    
                if area[1]<area[0]: #Area goes over 0:
                    ax.set_extent([area[0]-360-10, area[1]+10, 
                                  area[2]-10, area[3]+10, ], crs=ccrs.PlateCarree())
                else: 
                    ax.set_extent([area[0], area[1], 
                                  area[2], area[3], ], crs=ccrs.PlateCarree())
    ...
    def plot_dwts(self, anomaly = False):   
        ...
        for iv, var in enumerate(vars):  
            ...
            for d in range(n_dwts):
                ...
                ax.set_extent([lon_min, lon_max, ds.latitude.values.min()-5, ds.latitude.values.max()+5, ], crs=ccrs.PlateCarree())
