...
def Plot_GEVParams(xda_gev_var, c_shape='bwr', c_other='hot_r', show=True):
    ...
    for c, par in enumerate(params):
        ...
        if par == 'shape':
            ...
            lim = np.nanmax([np.abs(cl[0]), np.abs(cl[1])])
        ...
        if par == 'shape':
            cc = ax.pcolor(rr_pv, cmap=cma, vmin=-lim, vmax=lim, edgecolor='k')
        else:
            cc = ax.pcolor(rr_pv, cmap=cma, vmin=cl[0], vmax=cl[1], edgecolor='k')
